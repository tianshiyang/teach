 ...
 */
{
  strats.el = strats.propsData = function (parent, child, vm, key) {
    if (!vm) { // 只允许vue实例才拥有el属性，其他子类构造子类不允许有el属性
      warn(
...
function mergeDataOrFn (
  ...
  if (!vm) {
    ...
    if (!childVal) { // 子类不存在data选项，则合并结果为负累data选项
      return parentVal
    }
...
strats.data = function (
  ...
  // vm代表的是否为Vue创建的实例，否则是子父类的关系
  if (!vm) {
    if (childVal && typeof childVal !== 'function') { // 必须抱枕子类的data类型是一个函数而不是一个对象
      warn(
...
function mergeAssets (
  ...
  var res = Object.create(parentVal || null); // 创建一个空对象，其原型只想父类的资源选项
  if (childVal) {
